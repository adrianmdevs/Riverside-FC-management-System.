...
if(isset($add))
{
	$sql=mysqli_query($con,"select * from matches where match_no='$rno'");
	if(mysqli_num_rows($sql))
	{
	echo "this match is already added";	
	}		
	else
	{	
	$img=$_FILES['img']['name'];
	mysqli_query($con,"insert into matches values('','$rno','$type','$price','$details','$img')");	
...
<table class="table table-bordered">
	...
	<tr>
		<td colspan="2">
			<input type="submit" class="btn btn-primary" value="Add Match Details" name="add"/>
		</td>
	</tr>
</table> 
</form>